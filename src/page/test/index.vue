<script setup lang="ts">
import Cell from './cell.vue'
import usePlaceHook from '@/logic/place'

const {
  boatNum,
  boardListData,
  dragStart,
  dragging,
  dragEnd,
  handleDragEnter,
  handleDragLeave,
  handleDrop,
  handleTouchStart,
  handleTouchMove,
  handleTouchEnd,
  handleClick,
  randomPlace,
  submit,
  placeStatus,
} = usePlaceHook()
</script>

<template>
  <div class="flex items-center justify-around h-100vh w-800px overflow-auto m-auto">
    <div class="ml-10px h-500px w-250px flex flex-col gap-20px">
      <div
        v-if="boatNum.four"
        class="flex gap-5px"
      >
        <div
          id="four"
          draggable="true"
          class="w-200px h-50px boat"
          @click="handleClick"
          @touchstart="handleTouchStart"
          @touchmove.prevent="handleTouchMove"
          @touchend="handleTouchEnd"
          @dragstart="dragStart"
          @drag="dragging"
          @dragend="dragEnd"
        />
        <span>x{{ boatNum.four }}</span>
      </div>
      <div
        v-if="boatNum.three"
        class="flex gap-5px"
      >
        <div
          id="three"
          draggable="true"
          class="w-150px h-50px boat"
          @click="handleClick"
          @touchstart="handleTouchStart"
          @touchmove.prevent="handleTouchMove"
          @touchend="handleTouchEnd"
          @dragstart="dragStart"
          @drag="dragging"
          @dragend="dragEnd"
        />
        <span>x{{ boatNum.three }}</span>
      </div>
      <div
        v-if="boatNum.two"
        class="flex gap-5px"
      >
        <div
          id="two"
          draggable="true"
          class="w-100px h-50px boat"
          @click="handleClick"
          @touchstart="handleTouchStart"
          @touchmove.prevent="handleTouchMove"
          @touchend="handleTouchEnd"
          @dragstart="dragStart"
          @drag="dragging"
          @dragend="dragEnd"
        />
        <span>x{{ boatNum.two }}</span>
      </div>

      <div
        v-if="boatNum.one"
        class="flex gap-5px"
      >
        <div
          id="one"
          draggable="true"
          class="w-50px h-50px boat"
          @touchstart="handleTouchStart"
          @touchmove.prevent="handleTouchMove"
          @touchend="handleTouchEnd"
          @click="handleClick"
          @dragstart="dragStart"
          @drag="dragging"
          @dragend="dragEnd"
        />
        <span>x{{ boatNum.one }}</span>
      </div>

      <button
        class="h-50px"
        :disabled="placeStatus ==='placed'"
        @click="randomPlace"
      >
        随机摆放
      </button>
      <button
        class="h-50px"
        :disabled="boatNum.total>0 || placeStatus ==='placed'"
        @click="submit"
      >
        完成
      </button>
      <div class="h-200px h-150px h-100px opacity-0 position-fixed" />
    </div>
    <div
      id="board"
      class="flex flex-wrap w-500px h-500px bg-bluegray"
    >
      <Cell
        v-for="item, index in boardListData"
        :key="index"
        :cell-data="item"
        @enter="handleDragEnter"
        @leave="handleDragLeave"
        @place="handleDrop"
      />
    </div>
  </div>
</template>

<style scoped>
.boat {
  z-index: 1;
  background-color: #fbbf24;
}
</style>
